...
from jaxrl2.agents.cql_encodersep_parallel.critic_updater import update_critic, extend_and_repeat, reshape_for_cql_computation, get_cds_weights
...
from jaxrl2.agents.cql_encodersep_parallel.actor_updater_bc import log_prob_update_bc
...
from jaxrl2.networks.encoders.networks import Encoder, IdentityEncoder, PixelMultiplexer, PixelMultiplexerEncoder, PixelMultiplexerDecoder, AuxPixelMultiplexerDecoder
...
@functools.partial(jax.pmap, static_broadcasted_argnums=list(range(8,28)), axis_name='pmap')
def _update_jit(
        rng: PRNGKey, 
        actor: TrainState, 
        critic_encoder: TrainState,
        critic_decoder: TrainState, 
        target_critic_encoder_params: Params, 
        target_critic_decoder_params: Params, 
        temp: TrainState, 
        batch: TrainState,
        discount: float, 
        tau: float, 
        ...
        cross_norm:bool, 
        aug_next:bool,
        basis_projection_coefficient: float, 
        use_basis_projection: bool, 
        use_gaussian_policy: bool, 
        autoregressive_policy:bool, 
        min_q_version: int, 
        use_uds: int, 
        use_cds:int, 
        reward_min:float,
    ...
    if bc_phase:
        new_actor, actor_info = log_prob_update_bc(key, actor, new_critic_encoder, new_critic_decoder, batch)
        entropy = actor_info['bc_entropy']
    else:
    ...
    return rng, new_actor, (new_critic_encoder, new_critic_decoder), (new_target_critic_encoder_params, new_target_critic_decoder_params), new_temp, info_dict
...
class PixelCQLLearnerEncoderSepParallel(Agent):
    ...
    def __init__(self,
        ...
        elif policy_encoder_type == 'identity':
            print('using identity')
            policy_encoder_def = IdentityEncoder()
    ...
    def update(self, batch: FrozenDict):
        ...
        new_rng, new_actor, new_critic, new_target_critic_params, new_temp, info = _update_jit(
            ...
            self.autoregressive_policy, self.min_q_version, self.use_uds, self.use_cds, 
            self.min_reward, self.bc_start < self._curr_step < self.bc_end)
    ...
    def make_value_reward_visulization(self, variant, trajs, **kwargs):
        ...
        show_image = self.encoder_type not in ['identity']
        ...
        for itraj in range(num_traj):
            ...
            traj_images.append(make_visual(q_pred, rewards, observations['pixels'], masks, target_q_pred, bellman_loss, task_ids, q_pis, advantages, show_image=show_image))
...
def make_visual(q_estimates, rewards, images, masks, target_q_pred, bellman_loss, task_ids, q_pis, advantages, show_image=True):
    q_estimates_np = np.stack(q_estimates, 0)
    q_pis_np = np.stack(q_pis, 0)
    advantages_np = np.stack(advantages, 0)

    fig, axs = plt.subplots(9, 1, figsize=(8, 15))
    ...
    axs[2].plot(q_pis_np[:, 0], linestyle='--', marker='o')
    axs[2].plot(q_pis_np[:, 1], linestyle='--', marker='o')
    axs[2].set_ylabel('q pi values')
    
    axs[3].plot(advantages_np[:, 0], linestyle='--', marker='o')
    axs[3].plot(advantages_np[:, 1], linestyle='--', marker='o')
    axs[3].set_ylabel('advantages values')
    
    axs[4].plot(target_q_pred, linestyle='--', marker='o')
    axs[4].set_ylabel('target_q_pred')
    axs[4].set_xlim([0, len(target_q_pred)])
    ...
    if len(task_ids) > 0:
        axs[8].plot(task_ids, linestyle='--', marker='o')
        axs[8].set_ylabel('task_ids')
...
def make_visual_eval(q_estimates, rewards, images, masks=None, show_window=False, sel_images=True):
    if show_window:
        import matplotlib
